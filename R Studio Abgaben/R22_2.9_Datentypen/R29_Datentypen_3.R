...
textVerarbeiten <- function(z){
  starts_with_fake <- startsWith(z,"fake_")
  ergebnis <- z[!starts_with_fake]
  ergebnis <- gsub(pattern="dummy", replacement = "--", ergebnis)
  return(ergebnis)
}
...
gruppenMedian <- function(values, groups) {return(tapply(values, groups, median))}
